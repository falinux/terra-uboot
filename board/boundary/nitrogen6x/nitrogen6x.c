...
void dram_init_banksize(void)
{
    ...
    gd->bd->bi_dram[0].size = get_ram_size((void *)PHYS_SDRAM_1, PHYS_SDRAM_1_SIZE);
	gd->ram_size = gd->bd->bi_dram[0].size;

#if CONFIG_NR_DRAM_BANKS > 1
    gd->bd->bi_dram[1].start = PHYS_SDRAM_2;
    gd->bd->bi_dram[1].size = get_ram_size((void *)PHYS_SDRAM_2, PHYS_SDRAM_2_SIZE);
	gd->ram_size += gd->bd->bi_dram[1].size;
#endif

#if CONFIG_NR_DRAM_BANKS > 2
    gd->bd->bi_dram[2].start = PHYS_SDRAM_3;
    gd->bd->bi_dram[2].size = get_ram_size((void *)PHYS_SDRAM_3, PHYS_SDRAM_3_SIZE);
	gd->ram_size += gd->bd->bi_dram[2].size;
#endif

#if CONFIG_NR_DRAM_BANKS > 3
    gd->bd->bi_dram[3].start = PHYS_SDRAM_4;
    gd->bd->bi_dram[3].size = get_ram_size((void *)PHYS_SDRAM_4, PHYS_SDRAM_4_SIZE);
	gd->ram_size += gd->bd->bi_dram[3].size;
#endif

#if CONFIG_NR_DRAM_BANKS > 4
    gd->bd->bi_dram[4].start = PHYS_SDRAM_5;
    gd->bd->bi_dram[4].size = get_ram_size((void *)PHYS_SDRAM_5, PHYS_SDRAM_5_SIZE);
	gd->ram_size += gd->bd->bi_dram[4].size;
#endif

#if CONFIG_NR_DRAM_BANKS > 5
    gd->bd->bi_dram[5].start = PHYS_SDRAM_6;
    gd->bd->bi_dram[5].size = get_ram_size((void *)PHYS_SDRAM_6, PHYS_SDRAM_6_SIZE);
...
// [FALINUX - START]---------------------------------------------------------
#ifdef CONFIG_NADIA   //only nadia board
...
static iomux_v3_cfg_t const nadia_pads[] = {
	MX6_PAD_GPIO_18__GPIO7_IO13   | MUX_PAD_CTRL(NO_PAD_CTRL),	// Slot ID debug
	MX6_PAD_GPIO_19__GPIO4_IO05   | MUX_PAD_CTRL(NO_PAD_CTRL),	// Slot ID cpu

	MX6_PAD_NANDF_CS0__GPIO6_IO11 | MUX_PAD_CTRL(NO_PAD_CTRL),	// Rotary Switch_0
	MX6_PAD_NANDF_CS1__GPIO6_IO14 | MUX_PAD_CTRL(NO_PAD_CTRL),	// Rotary Switch_1
	MX6_PAD_NANDF_CS2__GPIO6_IO15 | MUX_PAD_CTRL(NO_PAD_CTRL),	// Rotary Switch_2
	MX6_PAD_NANDF_CS3__GPIO6_IO16 | MUX_PAD_CTRL(NO_PAD_CTRL),	// Rotary Switch_3

	MX6_PAD_GPIO_8__GPIO1_IO08   | MUX_PAD_CTRL(NO_PAD_CTRL),	// Buzzer Piezo	 
};
...
#else
#if defined(CONFIG_TERRA)
	puts("Board: EM-IMX6DQ\n");
#elif defined(CONFIG_NADIA)
